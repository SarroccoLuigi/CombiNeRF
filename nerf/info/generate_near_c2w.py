...
def get_near_pixel(coords, padding, H, W, device):
    '''
    ...
    set = torch.Tensor([1, -1, W, -W]).to(device=device, dtype=torch.int64)
    idx = torch.randint(low=0, high=4, size=(N,), dtype=torch.int64)
    delta = set[idx]

    near_coords = (coords + delta).clamp(0, H*W - 1)  # no others pixel number verification
    return near_coords
