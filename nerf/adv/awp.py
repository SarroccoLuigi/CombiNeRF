...
class AdvWeightPerturb(object):
    ...
    def calc_awp(self, opt, inputs, targets, delta, loss_fn, unadv=False):
        self.proxy.load_state_dict(self.model.state_dict())
        self.proxy.train()

        rays_o, rays_d, _, cam_id = inputs
        output = self.proxy.render(rays_o, rays_d, cam_id, adv_perturb=delta, **vars(opt))
        loss = -loss_fn(output['image'][:, :opt.num_rays], targets)
        if unadv:
            loss = -loss
